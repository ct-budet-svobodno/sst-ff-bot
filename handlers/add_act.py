...
async def add_act_report(call:CallbackQuery, state:FSMContext):
    await call.message.answer("🌸 Введите ФИО активиста: ")
    await call.answer()
...
async def add_smth(message:types.Message, state:FSMContext):
...
async def add_smth(message:types.Message, state:FSMContext):
    ...
    print(f'!!!{na}, {st}, {tgid}, {bd}, {sg}, {nu}, {em}, {stu}, {ot}')
    async for session in get_session():
        user = await add_activist(session, na, st, tgid, bd, sg, nu, em, stu, ot)
        if user is not None:
            await message.answer("Кажется, активист уже существует в базе!📍", reply_markup=return_to_start())
        else:
            await message.answer("Активист успешно добавлен в базу!✅", reply_markup=return_to_start())